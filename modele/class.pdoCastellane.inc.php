...
*/
    ...
    {
        $res = PdoCastellane::$monPdo->prepare('SELECT lecon.*, client.nom, moniteur.nom FROM `lecon`, client , moniteur 
		WHERE lecon.id_client = client.id_client and lecon.id_moniteur = moniteur.id_moniteur AND id_lecon = :id');
		$res->bindvalue('id', $id, PDO::PARAM_STR);
		$res->execute();
		$laLigne = $res->fetch();
		return $laLigne;
